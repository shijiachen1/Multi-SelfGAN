...
import models
import datasets
from function_controller_block import train, validate, LinearLrDecay, load_params, copy_params, get_reward_dis
...
def main():
    ...
    teacher_net_b.load_state_dict(checkpoint_256['gen_state_dict'])
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        ...
        train(args, gen_net, dis_net, teacher_net_b, teacher_net_dis, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
        EMs, staged_EMs_stu, entire_EMs_stu, entire_dis_staged_EM_stu, entire_fake_validity_stu, staged_fake_validity_stu, action_value = get_reward_dis(args, gen_net, teacher_net_b, teacher_net_dis, eval_loader)
        logger.info(f'tea_stu: {EMs}')
        logger.info(f'staged_dis_real_fake: {staged_EMs_stu}')
        logger.info(f'entire_dis_real_fake: {entire_EMs_stu}')
        logger.info(f'entire_dis_fake: {entire_fake_validity_stu}')
        logger.info(f'staged_dis_fake: {staged_fake_validity_stu}')
        logger.info(f'entire_dis_staged_EM_stu: {entire_dis_staged_EM_stu}')
