...
import models
import datasets
from function_controller_block import train_distill, validate, LinearLrDecay, load_params, copy_params
...
def main():
    ...
    teacher_net_b = eval('models.'+args.teacher_gen_b+'.Generator')(args=args).cuda()

    dis_net.cur_stage = 2

    checkpoint_file = args.teacher_gen_b_256_path
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        ...
        train_distill(args, teacher_net_b, gen_net, dis_net, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)
