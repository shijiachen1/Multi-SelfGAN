...
import models
import datasets
from function_controller_block import train_shared, train_distill, train_controller, get_topk_arch_hidden, train_KD, train_controller_block
...
def main():
    ...
    # state_dict_gen = teacher_net_b.state_dict()
    # state_dict_dis = teacher_net_dis.state_dict()

    # torch.save({'gen_state_dict': state_dict_gen}, checkpoint_file, _use_new_zipfile_serialization=False)
    # torch.save({'dis_state_dict': state_dict_dis}, checkpoint_file, _use_new_zipfile_serialization=False)


    teacher_net_b.load_state_dict(checkpoint['gen_state_dict'])
    teacher_net_dis.load_state_dict(checkpoint['dis_state_dict'])
    ...
    for search_iter in tqdm(range(int(start_search_iter), int(args.max_search_iter)), desc='search progress'):
        ...
        if search_iter % 15 == 0 and search_iter != 0:
            top_archs = get_topk_arch_hidden(args, controller1, controller2, controller3, gen_net)
        ...
        train_controller_block(args, controller1, controller2, controller3, ctrl_optimizer1, ctrl_optimizer2, ctrl_optimizer3,
                               teacher_net_b, teacher_net_dis, gen_net, eval_loader)
    ...
    final_archs = get_topk_arch_hidden(args, controller1, controller2, controller3, gen_net)
