...
class Cell(nn.Module):
    ...
    def forward(self, x, skip_ft=None):
        ...
        if self.up_mode == 'deconv':
            h = self.deconv(residual)
        else:
            h = F.interpolate(residual, scale_factor=2, mode=self.up_mode)
        h = self.c1(h)
        if self.norm:
            h = self.n1(h)
        _, _, ht, wt = h.size()
        ...
        # second conv
        if self.skip_in_ops:
            # print("len_skip_in_ops: {}, len_skip_ft: {}".format(len(self.skip_in_ops), len(skip_ft)))
