...
class Controller(nn.Module):
    ...
    def sample(self, batch_size, with_hidden=False, prev_hiddens=None, cur_top_hiddens=None):
        # x = self.initHidden(batch_size)
        if cur_top_hiddens:
            cur_hxs, cur_cxs = cur_top_hiddens
            cur_select_idx = np.random.choice(len(cur_top_hiddens) - 1, batch_size)  # TODO: replace=False
            cur_select_idx = [int(x) for x in cur_select_idx]

            cur_select_hxs = []

            for c_idx in cur_select_idx:
                cur_select_hxs.append(cur_hxs[c_idx].unsqueeze(0).detach())
            x = torch.cat(cur_select_hxs, 0)
        else:
            x = self.initHidden(batch_size)

        if prev_hiddens:
            prev_hxs, prev_cxs = prev_hiddens
            selected_idx = np.random.choice(len(prev_hiddens)-1, batch_size)  # TODO: replace=False
            ...
            for s_idx in selected_idx:
                selected_hxs.append(prev_hxs[s_idx].unsqueeze(0).detach())
                selected_cxs.append(prev_cxs[s_idx].unsqueeze(0).detach())
        ...
        actions = []
        selected_log_probs = []
        # print("cur_stage :{} len(decoders):{}".format(self.cur_stage, len(self.decoders)))
